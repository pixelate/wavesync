...
module Wavesync
  class CLITest < Wavesync::TestCase
    ...
    test 'syncs single device to its configured path' do
      @scanner.expects(:sync).with('/tmp/tp7', @device, pad: false, staged: false, mp3_bitrate: 192)
    ...
    test 'syncs only the selected device when multiple devices are configured' do
      ...
      @scanner.expects(:sync).with('/tmp/ot', @device, pad: false, staged: false, mp3_bitrate: 192).once
