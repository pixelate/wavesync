...
module Wavesync
  module Commands
    class Sync < Command
      ...
      THROTTLE_OPTION = Option.new(short: '-t', long: '--throttle MS', description: 'Wait MS milliseconds after each file is copied')

      self.name = 'sync'
      self.description = 'Sync music library to a device'
      self.options = [DEVICE_OPTION, PAD_OPTION, THROTTLE_OPTION].freeze
      ...
      def run
        options, config = parse_options(banner: 'Usage: wavesync sync [options]') do |opts, opts_hash|
          ...
          opts.on(*THROTTLE_OPTION.to_a) { |value| opts_hash[:throttle] = Integer(value) }
        end
        ...
        device_pairs.each do |pair|
          ...
          scanner.sync(device_config[:path], device, pad: options[:pad] || false, throttle: options[:throttle])
        end
      end
    end
  end
