namespace :checksum do
  ...
  task :compare, [:dir] do |_task, args|
    ...
    work_files.each do |file_path|
      ...
      if computed.nil?
        puts "%-30s %10s %10s %s" % [File.basename(file_path), "0x%02X" % stored, "n/a", "(no checksum)"]
      else
        match = stored == computed ? "OK" : "MISMATCH"
        puts "%-30s %10s %10s %s" % [File.basename(file_path), "0x%02X" % stored, "0x%02X" % computed, match]
      end
    end
  end
